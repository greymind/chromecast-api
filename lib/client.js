...
class Client extends EventEmitter {
  constructor () {
	...
	addDevice(name, friendlyName, host) {
    const oldDevice = this._devices[name];

    const isNewDevice = !oldDevice;
    const shouldUpdate = isNewDevice || oldDevice.name !== name || oldDevice.friendlyName !== friendlyName || oldDevice.host !== host;

		if (shouldUpdate) {
      this._devices[name] = {
        name: friendlyName, host
      }
